...
public class TokenManagerService
{
    ...
    /// <param name="state">從 callback query 取得的 state</param>
    public async Task GetTokenWithAuthorizationCode(string code, string? state = null)
    {
        ...
        // 6. 使用 OidcService 驗證 tokenNonce 是否存在於 cache（成功即刪除）
        //    - 若驗證失敗，將 _lastRefreshSuccess 設為 false，寫 log，並直接 return（不進行 sign-in）
        // 7. 若 nonce 驗證通過，照原流程建立 ClaimsIdentity 並 SignIn
